            ...
            <div class="card-body">
                ...
                <script>
                    ...
                    function editRow(idrow,dataid) {
                        //console.log(idrow);
                        getTableData('{{route("department.getdepartment")}}','post',dataid).then(
                        response =>{
                            data = response[0];

                            document.getElementById(idrow).innerHTML='<td colspan="1"></td><td><input type="text" class="form-control form-control-danger" id="name_update" value="'+data.name+'" oninput="this.value = this.value.toUpperCase()"></td><td colspan="4"><button class="btn btn-sm" type="button" onclick="updateRow('+"'name_update','post','{{route('department.updet')}}'"+','+"'"+data.id+"'"+')"><i class="fas fa-check text-primary font-18" aria-hidden="true"></i></button><button class="btn btn-sm" type="button" onclick="location.reload();"><i class="fas fa-times text-danger font-18" aria-hidden="true"></i></button></td>';
                ...
                <tr id="jsrow" style="display: none;">
                    ...
                    <td colspan="2">
                        <button type="button" class="btn btn-sm" onclick="addData('name','post','{{route('department.store')}}')" title="Add"><i class="fas fa-check text-info font-18" aria-hidden="true"></i></button>
                ...
                <tbody id="editTable">
                    ...
                    <tr id="row{{$i}}" >
                        ...
                        <td>{{$data->name}}</td>
                        <td>
                            <form action="{{route('department.update',$data->id) }}" name="sstc{{$data->id}}" id="sstc{{$data->id}}" method="post">
